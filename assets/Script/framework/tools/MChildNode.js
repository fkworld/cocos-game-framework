...
export default class MChildNode extends cc.Component {
    ...
    init_data() {
        ...
        for (let node of this.node_array_with_nodename) {
            if (node === null) { cc.error("[注意] 传入了一个值为null的子节点，node=", node); continue }
            if (this.object_node[node.name] != undefined) { cc.error("[注意] 传入了一个重复名字的子节点，node=", node); continue }
